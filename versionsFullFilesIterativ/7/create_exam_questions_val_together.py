...
def main():
    ...
    for index, question_content in enumerate(generated_tasks[SECTION_QUESTIONS], start=1):
        ...
        exam_question = ExamQuestionWithExampleAndTables(
            question_content=validated_task_and_tables[SECTION_QUESTION_CONTENT],
            example=validated_task_and_tables[SECTION_EXAMPLE],
            solution_state_transition_table=validated_task_and_tables[SECTION_SOLUTION_STATE_TRANSITION_TABLE],
            solution_example_flow_table=validated_task_and_tables[SECTION_SOLUTION_EXAMPLE_FLOW_TABLE]
        )
